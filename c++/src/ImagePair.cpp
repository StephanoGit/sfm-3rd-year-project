...
void AlignedPoints(std::vector<cv::Point2f> img1,
...
std::vector<Point_3D> ImagePair::triangulate(cv::Mat K, std::vector<double> d) {
    ...
    AlignedPoints(this->image1_good_matches, this->image2_good_matches, this->good_matches, al_img1, al_img2, img1_ref, img2_ref);
...
std::vector<Point_3D> ImagePair::init_reconstruction(FeatureDetectionType detection_type,
...
std::vector<Point_3D> ImagePair::get_points_3d() {
    return this->points_3d;
}
...
void ImagePair::set_P(cv::Mat P) {
    this->P = P;
}
cv::Mat ImagePair::get_P() {
    return this->P;
}