...
cv::Mat draw_matches(cv::Mat left, cv::Mat right, Features features_left,
  ...
  cv::drawMatches(left, features_left.key_points, right,
                  features_right.key_points, matches, matches_image,
...
//                                  pair.get_image1().get_image().cols +
//                                  pair.get_image2().get_image().rows,
...
//     cv::computeCorrespondEpilines(pair.get_image1_good_matches(), 1,
//     pair.get_F(), epilines1);
//     cv::computeCorrespondEpilines(pair.get_image2_good_matches(), 2,
//     pair.get_F(), epilines2);
//
...
//                  cv::Point(image1.cols, -(epilines1[i][2] + epilines1[i][0] *
//                  image1.cols) / epilines1[i][1]), color);
//
//         cv::circle(image1, pair.get_image2_good_matches()[i], 3,
//         cv::Scalar(0, 0, 255), -1);  // Blue points in img2
//
//         // Draw line on img2
//         cv::line(image2, cv::Point(0, -epilines2[i][2] / epilines2[i][1]),
//                  cv::Point(image2.cols, -(epilines2[i][2] + epilines2[i][0] *
//                  image2.cols) / epilines2[i][1]), color);
//
//         cv::circle(image2, pair.get_image1_good_matches()[i], 3,
//         cv::Scalar(0, 0, 255), -1);  // Green points in img1
//     }
//
