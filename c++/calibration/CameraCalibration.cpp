...
CameraCalibration::CameraCalibration(std::string directory, bool show_images,
                                     bool downscale)
    : checkerboard_size({6, 8}), square_size(23) {
...
void CameraCalibration::calibrateCamera(std::string directory,
                                        ...
                                        bool downscale) {
  std::vector<cv::Mat> images = load_images(directory, downscale);
  ...
  for (int i = 0; i < images.size(); i++) {
    ...
    if (show_images) {
      cv::drawChessboardCorners(
          images[i], cv::Size(checkerboard_size[0], checkerboard_size[1]), q[i],
          pattern_found);
  ...
  float error =
      cv::calibrateCamera(Q, q, frame_size, K, d, rvecs, tvecs, flags);
