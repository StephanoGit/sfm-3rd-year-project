...
CameraCalibration::CameraCalibration(std::string directory, bool show_images, int resize_val) : checkerboard_size({13, 9}), square_size(20) {
    calibrateCamera(directory, checkerboard_size, square_size, show_images, resize_val);
}

void CameraCalibration::calibrateCamera(std::string directory, std::vector<int> checkerboard_size, int square_size, bool show_images, int resize_val) {
    std::vector<cv::Mat> images = load_images(directory, resize_val);
    ...
    for (int i = 0; i < images.size(); i++) {
        ...
        pattern_found =
            cv::findChessboardCorners(gray, cv::Size(checkerboard_size[0], checkerboard_size[1]), q[i], cv::CALIB_CB_ADAPTIVE_THRESH + cv::CALIB_CB_NORMALIZE_IMAGE + cv::CALIB_CB_FAST_CHECK);
        if (pattern_found) {
        ...
        if (show_images) {
            ...
            for (int j = 0; j < checkerboard_size[1] - 1; j++) {
                for (int k = 0; k < checkerboard_size[0]; k++) {
                    ...
                    cv::line(images[i], current, below, cv::Scalar(0, 255, 0), 10);
                }
            }

            // Connect the last element of column i to the first corner of column i+1
            for (int k = 0; k < checkerboard_size[0] - 1; k++) {
                cv::Point last_of_column = q[i][(checkerboard_size[1] - 1) * checkerboard_size[0] + k];
                cv::Point first_of_next_column = q[i][k + 1];
                cv::line(images[i], last_of_column, first_of_next_column, cv::Scalar(0, 255, 0), 10);
            }
            // cv::drawChessboardCorners(images[i], cv::Size(checkerboard_size[0], checkerboard_size[1]), q[i], pattern_found);
